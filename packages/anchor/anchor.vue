<template>
  <div class="t-anchor-wrapper">
    <div class="t-anchor">
      <div class="t-anchor-ink">
        <t-icon-font v-if="icon" :name="icon" :style="iconStyle" :color="activeColorStr"></t-icon-font>
        <span v-show="!icon&&showInk" class="t-anchor-ink-ball"
              :style="{borderColor:activeColorStr, top: `${inkTop}px`}"></span>

        <span v-show="!icon&&!showInk" class="t-anchor-ink-line"
...
export default {
  ...
  methods: {
    ...
    updateTitleOffset() {
      const links = findComponentsDownward(this, 'TAnchorLink').map(link => {
        return link.href
      })
