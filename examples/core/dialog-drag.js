export default {
  ...
  bind(el, binding, vnode, oldVnode) {
    ...
    if (width.includes('%')) {
      width = document.body.clientWidth * (width.replace(/\%/g, '') * 1 / 100);
    } else {
      width = width.replace(/\px/g, '') * 1;
    }
    // dragDom.style.left = `${(document.body.clientWidth - width) / 2}px`;
    // 鼠标按下事件
    dialogHeaderEl.onmousedown = (e) => {
      // 拖拽时隐藏下拉项内容
      let popperEls = document.querySelectorAll('.el-popup-parent--hidden .el-popper');
      popperEls.forEach(p=>p.style.display='none');
      ...
      if (sty.left.includes('%')) {
        styL = document.body.clientWidth * (sty.left.replace(/\%/g, '') * 1 / 100);
        styT = document.body.clientHeight * (sty.top.replace(/\%/g, '') * 1 / 100);
      } else {
        styL = sty.left.replace(/\px/g, '') * 1;
        styT = sty.top.replace(/\px/g, '') * 1;
      };
